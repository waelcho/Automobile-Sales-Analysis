...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        df = recession_data.groupby(['unemployment_rate', 'Vehicle_Type'])['Automobile_Sales'].mean().reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(df, x='unemployment_rate', y="Automobile_Sales", color='Vehicle_Type',
    ...
    # Yearly Statistic Report Plots                             
        ...
        month = month.reindex(order, axis=0)
        month = month.reset_index()
        Y_chart2 = dcc.Graph(
            figure=px.line(month, x='Month', y='Automobile_Sales',
                title='Monthly sale for the year {}'.format(input_year)))
